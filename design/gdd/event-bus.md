...
| **Subscriber forgets to unsubscribe on scene unload** | Memory leak; stale handlers fire on destroyed objects | Provide a `scopedBus(cleanup)` helper that auto-unsubscribes all listeners when a cleanup function is called (tied to the runtime's scene dispose lifecycle). |
